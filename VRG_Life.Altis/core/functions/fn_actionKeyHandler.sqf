...
if((_curTarget isKindOf "Man" && !(_curTarget isKindOf "animal" ) ) && {!alive _curTarget}) exitWith {
	//Hotfix code by ins0
	if(((playerSide == blufor && {(call life_revive_cops)}) || playerSide == independent || (playerSide == civilian && license_civ_rebMedic)) && {"Medikit" in (items player)}) then {
...
// move this to the bottom since every other use case is more important to check first.
