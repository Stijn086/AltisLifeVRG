...
dokeyDown={
   ...
   // VARIOUS CHECKS 
   if (player getvariable["key",true] and (_this select 1)  == VAULTMOD_KEY) exitwith {player setvariable["key",false]; [_key_delay] spawn {sleep (_this select 0);player setvariable["key",true]; };_r};
     if ((_this select 1)  == VAULTMOD_KEY and speed player >8) then {
