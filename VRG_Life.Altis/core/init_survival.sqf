...
[] spawn
{
	...
	while{true} do
	{
		...
		if(playerSide == west) then {(unitBackpack player) setObjectTextureGlobal [0,""];}; // <--Hides Backpacks for cops
        if(playerSide == independent) then {(unitBackpack player) setObjectTextureGlobal [0,""];}; // <--Hides Backpacks for medics
