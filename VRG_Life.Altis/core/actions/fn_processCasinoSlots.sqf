...
_soundToPlay = _soundPath + "sounds\slot_spin.ogg";
_soundToPlay1 = _soundPath + "sounds\caching.ogg";
...
if(player distance _source > 2) exitWith 
{
	hint "You need to stay within 2m to play."; 
	//5 cutText ["","PLAIN"]; 
	life_is_processing = false;
};
...
switch (_type) do
{
	case 0:{_cost = 5000;};//penny snatcher's
	case 1:{_cost = 10000;};//average joe's
	case 2:{_cost = 15000;};//high roller's
};
_winAmount = 0;
...
if(life_cash >= _cost) then
{
	...
	playSound3D [_soundToPlay1, _source, false, getPos _source, 60, 1, 50];
	...
	playSound3D [_soundToPlay, _source, false, getPos _source, 60, 1, 50];
