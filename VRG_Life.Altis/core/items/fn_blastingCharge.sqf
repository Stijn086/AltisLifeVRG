...
if({side _x == west} count playableUnits < 7) exitWith {hint localize "STR_Civ_NotEnoughCops"};
{
	_rank = _x getVariable["rank",0];
    if((side _x == west) && (_rank > 4)) then 
	{
        _highrankcops = _highrankcops + 1;
    }; 
} forEach playableUnits;
if(_highrankcops<1) exitWith {hint localize "STR_Civ_NotEnoughCops"};
