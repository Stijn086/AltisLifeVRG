...
if (_pid!=getplayerUID player) exitWith { hint parseText "<t color='#FF0000' size='1'>The house inventory has been closed to prevent duping!";curHouse setVariable ["inUse",[false,""],true];closeDialog 0; };
	...
	switch (_mode) do {
			...
			_index = -1;
			{
				...
				if (_x select 0 == _selClassname1 AND _selClassname1 != "B_supplyCrate_F" AND _selClassname1 != "Box_IND_AmmoVeh_F" AND _selClassname1 != "Box_IND_WpsSpecial_F" AND _selClassname1 != "Box_IND_Grenades_F") then {
					...
					if (_canBeRemoved) then {
						...
						hint format ["You have no space in your inventory: %1",_item select 1];
					};
				};
			} forEach _content;
		};
		case 1 : {
			...
			if ((_boxslotsused+1) <= curHouseSpace) then {
				...
				if (true) exitWith {hint "The house is full!"};
			};
		};
	};
