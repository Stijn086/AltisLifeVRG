...
*/
private["_ret","_bad","_time","_bail","_esc","_countDown","_scale"];
_ret = [_this,0,[],[[]]] call BIS_fnc_param;
_bad = [_this,1,false,[false]] call BIS_fnc_param;
/* 
Mullsm0kes pro scaling system. Capped at 30 minutes. 
*/
_scale = 0;
if((_ret select 3) > 999) then { if((_ret select 3) < 20000) then {_scale = 120;} else {_scale = ((_ret select 3) / 1000) * 6;}; } else {_scale = 120;};
if(_scale > 1800) then {_scale = 1800;};
if(_bad) then { _time = time + (_scale * 1.15); } else { _time = time + _scale; };
if(count _ret > 0) then { life_bail_amount = (_ret select 3); } else { life_bail_amount = 1500; _time = time + 60; };
_esc = false;
_bail = false;
